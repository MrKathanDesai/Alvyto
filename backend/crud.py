...
def update_visit(db: Session, visit_id: str, updates: schemas.VisitUpdate):
    ...
    if updates.summary is not None:
        db_visit.summary = updates.summary.dict()

    if updates.status is not None:
        db_visit.status = updates.status
        # On approval, permanently erase transcript and dialogue.
        # Only the structured summary is kept as patient history.
        if updates.status == 'approved':
            db_visit.transcript = ''
            db_visit.dialogue = []

    if updates.ended_at is not None:
        db_visit.ended_at = updates.ended_at
...
def get_patient_visits(db: Session, patient_id: str):
    """Returns only approved visits for a patient — the permanent history record."""
    return (
